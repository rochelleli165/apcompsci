...
////////////////////////////////////////////////////////////////////////////////
/**
 * This class implements methods to manage the list of students waiting for their
 turn to get
 * assistance during consulting hours. The list of student requests is stored in an
 oversize array
 * defined by a reference to an array of strings which stores students requests and
 an int variable
 * which keeps track of the size of the array.
 *
 * The array requests is an oversize array. This means it contains non null
 references from index 0
 * to index size-1. All references in the range of indexes size .. requests.length-
 1 MUST be null.
 *
 * In this WaitingRoom class, new requests are added to the end of the array.
 Removal operations are
 * made from the beginning of the array.
 *
 * All requests are represented by strings formatted as follows: "studentName:
 keyword". The
 * studentName refers to the name of the students asking for assistance and the
 keyword represents
...
public class RequestList {
    /**
     * Adds a new request to the end of the requestList defined by requests and size.
     If requests
     * is full the new request won't be added to the requests array and the following
     message will be
     ...
     * @return the size of the oversize array after trying to insert newRequest to
    requests
     */
    public static int addLast(String[] requests, int size, String newRequest) {
        // TODO Complete the implementation of this method
        if(size == requests.length){
            System.out.println("Error! Full requests.");
            return size;
        }
     ...
     * @return the number of requests in requests which contain the string topic
    passed as input
     * @throws IllegalArgumentException if size is negative ( < 0)
     */
    public static int getRequestsCountByTopic(String[] requests, int size, String
            topic)
        ...
        for(int i = 0; i < size; i++){
            int space = requests[i].indexOf(" ");
            String request = requests[i].substring(space+1);
    ...
    /**
     * Removes the student request stored at index zero of the oversize array defined
     by requests and
     * size.
     *
     * If the array is empty (its size is zero), this method prints the following
     message:
     *
     ...
     * @return the size of the oversize array after trying to remove the request at
    index 0.
     */
    ...
    // was 4, after calling removeFirst(requests, size) the array should contain {e1, e2, e3, null,
        ...
        for(int i = 0; i < size; i++){
            String nextRequest = requests[i+1];
    ...
    /**
     * Checks whether the correctness of the RequestList.removeFirst() method when
     passed a non-empty
     * oversize array.
     *
    ...
    public static boolean testSuccessfulRemoveFirst() {
        ...
        String[] requests = new String[] {"Michelle: p1", "Mouna: p2", "Hobbes: p2",
                "Ashley: quiz1-2", null, null, null, null};
        int size = 4;
        size = removeFirst(requests, size);
        String[] arrResult = new String[] {"Mouna: p2", "Hobbes: p2",
                "Ashley: quiz1-2", null, null, null, null, null};
        if(!Arrays.deepEquals(requests, arrResult)){
    ...
    /**
     * Checks whether the RequestList.addLast method works as expected when called to
     add a new
     ...
     * Checks the correctness of the implementation of the
     RequestList.getRequestsCountByTopic()
    ...
    public static boolean testGetRequestsCountByTopicNotFullList() {
        ...
        String[] originalRequests = new String[] {"Mouna: p2", "Hobbes: javadocs",
                "Ashley: p2",
    ...
    public static void main(String[] args) {
        System.out.println(
                "testGetRequestsCountByTopicNotFullList()): " +
                        testGetRequestsCountByTopicNotFullList());
        System.out.println("testSuccessfulRemoveFirst(): " +
                testSuccessfulRemoveFirst());
